...
// --- TU CONFIGURACIÓN ---
const APP_CONFIG = {
  timezone: 'America/Argentina/Buenos_Aires', // Forzamos siempre Argentina
  locale: 'es-AR'
};

// --- AYUDANTE DE FECHAS (MAGIA) ---
// Convierte cualquier fecha a YYYY-MM-DD respetando la zona horaria de Argentina
...
export default function PlanificacionPage() {
  ...
  const loadGridData = async () => {
    ...
    // Usamos string para asegurar rango completo sin problemas de hora
  ...
  const getDaysToRender = () => {
    ...
    if (viewMode === 'week') {
      ...
      for (let i = 0; i < 7; i++) { const current = new Date(start); current.setDate(start.getDate() + i); days.push({ dateStr: current.toISOString().split('T')[0], label: current.toLocaleDateString(APP_CONFIG.locale, { weekday: 'short', day: 'numeric' }), dayObj: current, dayName: ['Do','Lu','Ma','Mi','Ju','Vi','Sá'][current.getDay()], dayNumber: current.getDate() }); }
    } else {
  ...
  const getShiftCode = (shift) => {
    ...
    // Usamos el helper para no errarle a la zona horaria al mostrar el turno
    const dateStr = shift.startTime?.seconds 
      ? new Date(shift.startTime.seconds * 1000).toLocaleTimeString('en-US', { hour12: false, timeZone: APP_CONFIG.timezone })
      : new Date(shift.startTime).toLocaleTimeString('en-US', { hour12: false, timeZone: APP_CONFIG.timezone });
  ...
  const title = viewMode === 'week' 
    ...
    : currentDate.toLocaleString(APP_CONFIG.locale, { month: 'long', year: 'numeric' }).toUpperCase();
              ...
              {filteredEmployees.map(emp => (
                <div key={emp.id} className="h-10 flex border-b">
                  {daysToRender.map(day => {
                    const dailyShifts = shifts.filter(s => s.employeeId === emp.id && toDateStr(s.startTime) === day.dateStr); // Usamos helper
                    ...
                    // --- AQUÍ ESTÁ LA SOLUCIÓN DEFINITIVA DE LICENCIAS ---
                    // Convertimos todo a texto "YYYY-MM-DD" de Argentina y comparamos strings. Infalible.
