...
console.log(`\n📚 INSTALANDO DOCUMENTADOR DE ARQUITECTURA (FINAL)`);

// --- RUTAS ---
const BASE_PATH = path.join('apps', 'web2', 'src');
const COMPONENT_DIR = path.join(BASE_PATH, 'components', 'dev');
const COMPONENT_FILE = path.join(COMPONENT_DIR, 'AutoSchemaDoc.tsx');
const PAGE_DIR = path.join(BASE_PATH, 'pages');
const PAGE_FILE = path.join(PAGE_DIR, 'test-schema.tsx');

// Crear directorios si no existen
if (!fs.existsSync(COMPONENT_DIR)) fs.mkdirSync(COMPONENT_DIR, { recursive: true });
if (!fs.existsSync(PAGE_DIR)) fs.mkdirSync(PAGE_DIR, { recursive: true });

// --- DATOS PROVISTOS POR EL USUARIO ---
const REAL_COLLECTIONS = [
    'audit_logs', 'auditorias', 'audits', 'ausencias', 'clients', 
    'config', 'convenios_colectivos', 'empleados', 'feriados', 
    'historial_operaciones', 'historial_resoluciones', 'novedades', 
    'planificaciones_historial', 'roles', 'servicios_sla', 
    'system_users', 'tipos_turno', 'turnos'
];

const REAL_FUNCTIONS = [
    { name: 'manageSystemUsers', url: 'https://us-central1-comtroldata.cloudfunctions.net/manageSystemUsers' },
    { name: 'crearUsuarioSistema', url: 'https://us-central1-comtroldata.cloudfunctions.net/crearUsuarioSistema' },
    { name: 'registrarFichadaManual', url: 'https://us-central1-comtroldata.cloudfunctions.net/registrarFichadaManual' },
    { name: 'checkSystemHealth', url: 'https://us-central1-comtroldata.cloudfunctions.net/checkSystemHealth' },
    { name: 'manageData', url: 'https://us-central1-comtroldata.cloudfunctions.net/manageData' },
    { name: 'manageAgreements', url: 'https://us-central1-comtroldata.cloudfunctions.net/manageAgreements' },
    { name: 'manageHierarchy', url: 'https://us-central1-comtroldata.cloudfunctions.net/manageHierarchy' },
    { name: 'managePatterns', url: 'https://us-central1-comtroldata.cloudfunctions.net/managePatterns' },
    { name: 'manageShifts', url: 'https://us-central1-comtroldata.cloudfunctions.net/manageShifts' },
    { name: 'auditShift', url: 'https://us-central1-comtroldata.cloudfunctions.net/auditShift' },
    { name: 'manageEmployees', url: 'https://us-central1-comtroldata.cloudfunctions.net/manageEmployees' },
    { name: 'manageAbsences', url: 'https://us-central1-comtroldata.cloudfunctions.net/manageAbsences' },
    { name: 'createUser', url: 'https://us-central1-comtroldata.cloudfunctions.net/createUser' },
    { name: 'limpiarBaseDeDatos', url: 'https://us-central1-comtroldata.cloudfunctions.net/limpiarBaseDeDatos' },
    { name: 'manageAudits', url: 'https://us-central1-comtroldata.cloudfunctions.net/manageAudits' },
    { name: 'scheduleShift', url: 'https://us-central1-comtroldata.cloudfunctions.net/scheduleShift' },
    { name: 'reportarAusencia', url: 'https://us-central1-comtroldata.cloudfunctions.net/reportarAusencia' }
];
...
const COLLECTIONS = ${JSON.stringify(REAL_COLLECTIONS)};
const FUNCTIONS = ${JSON.stringify(REAL_FUNCTIONS)};
...
import { AutoSchemaDoc } from '@/components/dev/AutoSchemaDoc';
export default function TestSchemaPage() { return <AutoSchemaDoc />; }
`;

// --- ESCRITURA ---
try {
    fs.writeFileSync(COMPONENT_FILE, componentCode);
    console.log(`✅ Componente actualizado: ${COMPONENT_FILE}`);
    fs.writeFileSync(PAGE_FILE, pageCode);
    console.log(`✅ Página lista: ${PAGE_FILE}`);
    console.log(`\n🎉 INSTALACIÓN COMPLETA.`);
    console.log(`👉 Ejecuta 'npm run dev' y ve a: http://localhost:3000/test-schema`);
} catch (e) {
